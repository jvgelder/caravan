...
import { autoLoadPSBT } from "@caravan/bitcoin";
...
const BIP_370_VECTORS_VALID_PSBT = [
  ...
  // Case: 1 input, 2 output PSBTv2, required fields only.
  {
    ...
    v0UnsignedRawTransaction:
      "02000000010b0ad921419c1c8719735d72dc739f9ea9e0638d1fe4c1eef0f9944084815fc80000000000ffffffff020008af2f00000000160014c430f64c4756da310dbd1a085572ef299926272c8bbdeb0b000000001600144dd193ac964a56ac1b9e1cca8454fe2f474f851300000000",
    inputs: 1,
    outputs: 2,
  },
  ...
  // Case: 1 input, 2 output updated PSBTv2, with PSBT_IN_SEQUENCE.
  {
    ...
    v0UnsignedRawTransaction:
      "02000000010b0ad921419c1c8719735d72dc739f9ea9e0638d1fe4c1eef0f9944084815fc80000000000feffffff020008af2f00000000160014c430f64c4756da310dbd1a085572ef299926272c8bbdeb0b000000001600144dd193ac964a56ac1b9e1cca8454fe2f474f851300000000",

    // ""
    inputs: 1,
    outputs: 2,
  ...
  // Case: Fallback locktime of 0 (computed timelock should be 0).
  {
    ...
    v0UnsignedRawTransaction:
      "02000000020f758dbfbd4da7c16c8a3309c3c81e1100f561ea646db5b01752c485e1bdde9f0100000000ffffffff3a1b3b3c837d6489ea7a31d8e6c7dd503c001bef3e06958e7574808d68ca78a50000000000ffffffff014f933577000000001600140b1352cacd03cf6aa1b7f3c8d6388671b34a5e1100000000",
    inputs: 2,
    outputs: 1,
  },
...
const BIP_174_VECTORS_VALID_PSBT = [
  ...
  // Case: PSBT with `PSBT_GLOBAL_XPUB`.
  {
    ...
    unsignedRawTx:
      "",
    inputs: 2,
    outputs: 2,
  },
  ...
  // Case: PSBT with global unsigned tx that has 0 inputs and 0 outputs
  {
    ...
    unsignedRawTx: "00000000000000000000",
    inputs: 0,
    outputs: 0,
  },
  ...
  // Case: PSBT with 0 inputs
  {
    ...
    unsignedRawTx:
      "020000000002d3dff505000000001976a914d0c59903c5bac2868760e90fd521a4665aa7652088ac00e1f5050000000017a9143545e6e33b832c47050f24d3eeb93c9c03948bc787b32e1300",
    inputs: 0,
    outputs: 2,
  },
];
...
describe("PsbtV2.toV0", () => {
  // A PSBT is still valid if it includes unrecognized key-value pairs, so we
  // don't need to check that the incompatible keys were removed, but we do need
  // to check that the required PSBT_GLOBAL_UNSIGNED_TX was properly created.

  test.each(BIP_174_VECTORS_VALID_PSBT)(
    "Properly constructs PSBT_GLOBAL_UNSIGNED_TX from BIP174 vectors. $case",
    (vect) => {
      const psbtv2 = PsbtV2.FromV0(vect.base64, true);
      const psbtv0Serialized = psbtv2.toV0("hex");
      // At a minimum, the raw tx cannot start before string index 16 because
      // the hex psbt must at least start with "70736274ff0100" and one byte for
      // length.
      expect(
        psbtv0Serialized.indexOf(vect.unsignedRawTx),
      ).toBeGreaterThanOrEqual(16);

      const psbtv0FromAutoLoadPsbt = autoLoadPSBT(psbtv0Serialized);
      const unsignedTxn = (
        psbtv0FromAutoLoadPsbt as unknown as Psbt
      ).data.globalMap.unsignedTx
        .toBuffer()
        .toString("hex");
  ...
  test.each(BIP_370_VECTORS_VALID_PSBT)(
    "Properly constructs PSBT_GLOBAL_UNSIGNED_TX from BIP370 vectors. $case",
    (vect) => {
      const psbtv2 = new PsbtV2(vect.base64);
      const psbtv0Serialized = psbtv2.toV0("hex");
      // At a minimum, the raw tx cannot start before string index 16 because
      // the hex psbt must at least start with "70736274ff0100" and one byte for
      // length.
      expect(
        psbtv0Serialized.indexOf(vect.v0UnsignedRawTransaction),
      ).toBeGreaterThanOrEqual(16);
